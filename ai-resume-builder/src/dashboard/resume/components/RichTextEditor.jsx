...
function RichTextEditor({ onRichTextEditorChange, index, defaultValue }) {
  ...
  console.log(
    "Data dari resumeInfo.experience[index]:",
  ...
  const GenerateSummeryFromAI = async () => {
    ...
    const cleanText = text
      .replace(/^["[{]+|["}\]]+$/g, '') // Hapus karakter awal/akhir seperti ", [, ]
      .split('", "') // Pisahkan tiap item
      .map(item => `<li>${item.replace(/^"|"$/g, '')}</li>`) // Buat jadi list
      .join('');
    
    setValue(`<ul>${cleanText}</ul>`);
